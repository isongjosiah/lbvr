...
\begin{figure}[!t]
  ...
  \caption{E1: ingest throughput by corpus scale. 100\,K projection extrapolates 10\,K steady-state.}
  \label{fig:e1}
\end{figure}
...
\begin{figure}[!t]
  ...
  \caption{E2: retrieval latency CDF for LBVR-Med vs four baselines.}
  \label{fig:e2}
\end{figure}
...
Figure~\ref{fig:e4} measures time-to-availability post-PUT, polled at logarithmic checkpoints (100\,ms~$\to$~5\,min). Hot reaches 99\% reachability within 510\,ms; warm within 2\,s; cold within 120\,s. All bundles eventually reachable (zero polling-budget timeouts at $n=100$). The cold-tier tail is the chain-settlement signature; clinical workflows are bounded by the hot+warm path.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{E4_tta_curves}
  \caption{E4: time-to-availability post-PUT, by tier (sim-mode calibration).}
  \label{fig:e4}
\end{figure}

PoR proof cost (Figure~\ref{fig:e5}) measures responder-side prove time and contract-side verify gas across Merkle proof depths $\{3, 5, 7, 9, 11, 13\}$ (covering 128\,KiB to 128\,MiB bundles). Prove time is constant at $\approx 630\,\mu$s P50 across all depths---BLS sign dominates, Merkle-proof construction contributes \(< 2\,\mu\)s. Gas for \texttt{respondToChallenge} grows linearly at 12\,k gas per depth level (290\,k at depth 3, 533\,k at depth 13); \texttt{postChallenge} and \texttt{recordVerdict} are depth-independent at 122\,k and 75\,k respectively. A full PoR cycle at depth 13 is 731\,k gas combined.
...
\begin{figure}[!t]
  ...
  \caption{E6 uniform Byzantine: smooth degradation as adversary fraction $f$ increases.}
  \label{fig:e6}
\end{figure}
...
\begin{figure}[!t]
  ...
  \caption{E6b tier-selective Byzantine: detection-gap up to $f=0.33$, sharp cliff to 0\% at $f=0.67$.}
  \label{fig:e6b}
\end{figure}
...
\begin{figure}[!t]
  ...
  \caption{E9: single-tier failure recovery CDF. Cold-tier wait dominates D0/D1 recovery.}
  \label{fig:e9}
\end{figure}
...
\begin{figure}[!t]
  ...
  \caption{E9-multi: double-tier failure detection. Early-fail shorts D0+D1 to ~137\,ms median.}
  \label{fig:e9multi}
\end{figure}
...
Figure~\ref{fig:e7} compares monthly storage cost across five architectures at the 100\,K-corpus scale (430.25\,GiB). LBVR-Med's RS(2,3) layout costs \$50.55/month, 5.1$\times$ cheaper than naive 3$\times$ Pinata replication (\$258/mo) and within 1.7$\times$ of S3 3-region replication while delivering decentralized governance, on-chain provenance, and Byzantine withstand the latter lacks.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{E7_storage_cost}
  \caption{E7: monthly storage cost by architecture, 100\,K-corpus scale.}
  \label{fig:e7}
\end{figure}

The end-to-end SLO attainment table (Figure~\ref{fig:e8}) synthesises Sections~\ref{sec:eval:ingest}--\ref{sec:eval:tta-por}: the IEC~60601-1-8 alarm SLO (10\,s) PASSES with margin; clinical radiology open (2\,s) lands in STRETCH territory under WAN adversity; chart-pull (500\,ms) FAILS under WAN stress and lands at the SLO boundary at baseline. The composite verdict per SLO ignores ingest (decouplable via pre-staging) and rolls up retrieval-baseline + stress + TTA via strictest-wins.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{E8_slo_attainment}
  \caption{E8: end-to-end SLO attainment with verdict per axis.}
  \label{fig:e8}
\end{figure}

Cross-SLO calibration (Figure~\ref{fig:e10}) places the LBVR-Med retrieval distribution against five domain SLO thresholds spanning four orders of magnitude. Power-grid GOOSE (4\,ms, IEC~61850) is 0.0\% feasible---no decentralized verifiable storage configuration meets a sub-10\,ms budget when even AES-GCM open + Merkle re-verify costs $\sim$5\,ms on commodity x86. The clinical and scientific-DT regimes (radiology open 2\,s and the interTwin notional 5\,s budget) are met at 100\%. The chart-pull 500\,ms boundary sits at 98.2\% feasibility; this is the limiting clinical workflow and the load-bearing finding for Section~\ref{sec:discussion}.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{E10_cross_slo_calibration}
  \caption{E10: cross-domain SLO calibration. Power-grid GOOSE infeasible; clinical/DT met.}
  \label{fig:e10}
\end{figure}
