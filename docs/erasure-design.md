...
For Synthea bundles (measured P50 ≈ 2.23 MB, P95 ≈ 14.9 MB on Synthea 3.3 / US Core 6.1.0; full 100K corpus, see `eval/results/synthea-100000/validation.json`), bundle-level recovery downloads over hot + warm tiers remain well under SLO, so Option B still wins decisively — but by a narrower margin than the original "~800 KB median" scoping suggested, and the ≥ 5 MB tail (11.3% of bundles, up to ~98 MB max) will stress the reconstruction path more than initially modelled. Option A would be justified only for very large bundles (e.g., DICOM imaging in the journal extension), which is noted as journal-scope optimization.
