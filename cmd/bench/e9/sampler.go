// Bundle sampling for E9. Decision (CLAUDE.md §10 D9, docs/eval-protocol.md §3,
...
// proportional sampling because every row is one observation. Honest
// representation; stratified-to-§4.5-bands would clip the 11.3% ≥ 5 MB tail.
...
func SampleBundles(sizesCSV string, n int, seed int64) ([]SampledBundle, error) {
	...
	// Reservoir-style would work but we have all rows in memory; just
	// shuffle then slice. Still O(n) selection cost.
...
// pointer if the file is missing — D9 hard-blocks otherwise.
